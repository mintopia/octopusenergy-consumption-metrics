    ...
    while (true){
        ...
        for await (var electric of electricresponse.data.results) {
            ...
            let electricpoint = new Point('electricity')
                .floatField('consumption', Number(electric.consumption))
                .timestamp(nanoDate);
            writeApi.writePoint(electricpoint);

            if(OCTO_ELECTRIC_COST) {
                let electriccost = Number(electric.consumption) * Number(OCTO_ELECTRIC_COST) / 100;
        ...
        for await (var gas of gasresponse.data.results) {
            const ts = new Date(gas.interval_end);
            const nanoDate = toNanoDate(String(ts.valueOf()) + '000000');

            let gaspoint = new Point('gas')
                .floatField('consumption', Number(gas.consumption))
                .timestamp(nanoDate);
            writeApi.writePoint(gaspoint);

            if(OCTO_GAS_COST) {
                let gascost = Number(gas.consumption) * Number(OCTO_GAS_COST) / 100;
